...
import { ArrowRight, Bell, Map, MapPin, Search, Tag } from 'lucide-react';
...
export default function MobileHome({ featuredMotels, cities }: { featuredMotels: PublicMotelListItem[]; cities: Array<{ name: string; total: number }> }) {
  ...
  return <main className="bg-slate-50 md:hidden">
    ...
    {cities.length > 0 && <section className="px-4 py-3"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-bold text-slate-900">Moteles por ciudad</h2><Link href="/search" className="text-sm font-semibold text-purple-600">Ver todos</Link></div><div className="flex flex-wrap gap-2">{cities.map((city) => <Link key={city.name} href={`/ciudad/${encodeURIComponent(city.name.toLowerCase().replace(/\s+/g, '-'))}`} className="rounded-full bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm">{city.name} <span className="text-slate-400">({city.total})</span></Link>)}</div></section>}
  </main>;
}
