...
type Report = {
  ...
  review: { id: string; score: number; comment: string | null; createdAt: string; isAnonymous: boolean; user: Person | null } | null;
};
...
const labels: Record<string, string> = {
  ...
  REVIEW_SPAM: 'Reseña: spam', REVIEW_OFFENSIVE: 'Reseña: contenido ofensivo', REVIEW_PRIVACY: 'Reseña: datos personales', REVIEW_FALSE: 'Reseña: información falsa', REVIEW_OTHER: 'Reseña: otro motivo',
};
...
export default function ReportsPage() {
  ...
  const removeReportedReview = async () => {
    if (!selected?.review || !window.confirm('¿Eliminar definitivamente la reseña denunciada?')) return;
    setSaving(true);
    const response = await fetch(`/api/admin/reviews/${selected.review.id}`, { method: 'DELETE' });
    if (response.ok) {
      await update({ status: 'RESOLVED', resolutionSummary: resolutionSummary.trim() || 'Reseña eliminada por moderación.' });
    } else {
      setSaving(false);
    }
  };
  ...
  return (
    <main className="space-y-6 p-4 md:p-8">
      ...
      <section className="grid gap-3 rounded-2xl border border-violet-100 bg-white p-4 shadow-sm md:grid-cols-[1fr_220px]">
        <input aria-label="Buscar reportes" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por motel o contenido de reseña" className="min-h-11 rounded-xl border border-slate-200 px-4 outline-none focus:border-violet-500" />
